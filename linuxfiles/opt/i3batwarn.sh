...
# FORCING TO BAT0 which is the internal battery that runs last
BATTERY=BAT0
...
FULL=`grep -v "DESIGN" $ACPI_PATH/uevent | grep "POWER_SUPPLY_ENERGY_FULL" | cut -d= -f2`

# get current energy value in percent
PERCENT=`echo $(( $REM * 100 / $FULL ))`

# set error message
MESSAGE="WARNING Battery at "$PERCENT"%"
